/**
 * Hämtar föreningens egna texter från SvenskaFans.
 *
 * RSS-flödet bär bara rubrik, länk och datum. Brödtexten hämtas därför ur
 * varje artikelsidas `#article-content`, skribenten och datumet ur sidans
 * JSON-LD, och bilden ur og:image.
 *
 * Delas av kommandoradsskriptet och av den skyddade endpointen, så att en
 * import gör samma sak oavsett varifrån den startas.
 */
...
export type Feed = { title: string; link: string; pubDate: string }
...
/**
 * Läser in en artikel. Matchningen sker på den ursprungliga länken, så en
 * andra körning uppdaterar i stället för att skapa dubbletter.
 */
export async function importArticle(payload: Payload, item: Feed): Promise<ImportResult> {
  let article: Article | null = null
  try {
    article = parseArticle(await fetchText(item.link), item)
  } catch (err) {
    return { status: 'skipped', title: item.title, reason: (err as Error).message }
  }
  if (!article) {
    return {
      status: 'skipped',
      title: item.title,
      reason: 'poddavsnitt eller text utan stycken',
    }
  }

  // Bilden laddas in i mediabiblioteket så att sajten inte blir beroende av
  // att SvenskaFans fortsätter serva den.
  ...
  // säsonger, så slugen härleds ur artikelns id hos SvenskaFans.
  ...
  try {
    ...
    return { status: 'skipped', title: article.title, reason: (err as Error).message }
  }
}
